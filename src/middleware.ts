...
export function middleware(request: NextRequest) {
  ...
  if (isSubdomainAdmin) {
    ...
    const isVercelOrLocal = hostname.endsWith('.vercel.app') || hostname.includes('localhost') || hostname.includes('127.0.0.1') || hostname.includes('brainket.online') || hostname.includes('growffi.com');
