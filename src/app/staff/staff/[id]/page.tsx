...
export default function EditStaffPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', mobile: '', userId: '', password: '', status: 'active' });
    ...
    const fetchStaff = async () => {
      try {
        ...
        if (res.success && res.staff) {
          const s = res.staff;
          setForm({ name: s.name, email: s.email, mobile: s.mobile || '', userId: s.userId, password: '', status: s.status });
          const existingPerms: any[] = s.permissions || [];
          setPermissions(
            getDefaultPermissions().map((def) => {
              const found = existingPerms.find(
                (p: any) => p.module === def.module && p.permission === def.permission
              );
              return { ...def, granted: found?.granted || false };
            })
          );
        }
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.put(`/api/admin/staff/${id}`, updateData);
      if (res.success) {
  ...
  return (
    <div>
      <div className="page-header" style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <Link href="/staff/staff" style={{ color: 'var(--text-muted)' }}><ArrowLeft size={18} /></Link>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-heading)' }}>Edit Staff: {form.name}</h2>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Card>
          ...
          <div className="form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 6 }}>New Password (leave blank to keep current)</label>
        ...
        <Card style={{ marginTop: 16 }}>
          ...
          <div className="perm-table" style={{
                ...
                >
                  ...
                  <div style={{
                    ...
                    <span style={{
                      ...
                      color: anyGranted ? 'var(--primary)' : 'var(--text-heading)',
                    }}>
                      {def.label}
                    </span>
