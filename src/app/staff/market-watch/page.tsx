...
export default function MarketWatchPage() {
  const [canLiveView, setCanLiveView] = React.useState(false);
  const [canHistoricalView, setCanHistoricalView] = React.useState(false);
  const [canExportMW, setCanExportMW] = React.useState(false);
  const [permsReady, setPermsReady] = React.useState(false);

  const [viewMode, setViewMode] = React.useState<'live' | 'historical'>('live');

  React.useEffect(() => {
    const lv = hasPerm('marketWatch', 'liveView');
    const hv = hasPerm('marketWatch', 'historicalView');
    const ex = hasPerm('marketWatch', 'export');
    setCanLiveView(lv);
    setCanHistoricalView(hv);
    setCanExportMW(ex);
    ...
    // Choose appropriate default mode based on permission options
    if (lv) {
      setViewMode('live');
    } else if (hv) {
  ...
  // Also try to refresh permissions from API on mount
  React.useEffect(() => {
    const storedStaffId = typeof window !== 'undefined' ? localStorage.getItem('growffiy_staff_id') : null;
    if (!storedStaffId) return;
    fetch(`/api/admin/staff/${storedStaffId}`)
      .then(r => r.json())
      .then(res => {
        if (!res.success || !res.staff?.permissions) return;
        const perms = res.staff.permissions;
        localStorage.setItem('growffiy_staff_permissions', JSON.stringify(perms));
        
        // Check new permission mappings
        const lv = perms.some((p: any) => p.module === 'marketWatch' && p.permission === 'liveView' && p.granted);
        const hv = perms.some((p: any) => p.module === 'marketWatch' && p.permission === 'historicalView' && p.granted);
        ...
        // Choose appropriate default mode based on fresh permissions
        if (lv) {
          setViewMode('live');
        } else if (hv) {
          setViewMode('historical');
  ...
  if (!permsReady) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px', gap: '16px' }}>
        <div style={{ color: 'var(--text-secondary)', fontWeight: 500, fontSize: '15px' }}>Loading...</div>
      </div>
    );
  }
